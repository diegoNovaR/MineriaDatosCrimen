...
def _verificar_datasets(datasets: dict) -> bool:
    if not datasets:
        print("\n  [AVISO] No hay datasets cargados. Usa la opción 1 o 2 primero.")
        return False
    return True
...
def main():
    ...
    while True:
        ...
        if opcion == "1":
            datasets = cargar_todos(anio=2025)
            print(f"\n  ✔ Datasets cargados: {list(datasets.keys())}")

        elif opcion == "2":
            resultado = cargar_individual(anio=2025)
            datasets.update(resultado)
            if resultado:
                print(f"\n  ✔ Dataset cargado: {list(resultado.keys())[0]}")

        elif opcion == "3":
            if _verificar_datasets(datasets):
                load_analyzer.analizar(datasets)

        elif opcion == "4":
            if _verificar_datasets(datasets):
                column_comparator.comparar(datasets)
